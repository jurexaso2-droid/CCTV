...
export default defineComponent({
  name: "Generate Termux CCTV Setup",
  description: "Creates a complete package of scripts and files to turn an old Android phone into a home security camera system using Termux",
  type: "action",
  props: {
    serverPort: {
      type: "integer",
      label: "Server Port",
      description: "Port number for the camera server",
      default: 8080,
      min: 1024,
      max: 65535
    },
    cameraResolution: {
      ...
      description: "Camera recording resolution",
      options: [
    ...
    streamQuality: {
      ...
      description: "Video stream quality setting",
      options: [
    ...
    recordingEnabled: {
      ...
      description: "Enable automatic video recording",
      default: true
    },
    adminPassword: {
      type: "string",
      label: "Admin Password",
      description: "Password for accessing the camera interface",
      secret: true,
      default: "admin123"
    }
  },
...
const ADMIN_PASSWORD = "${this.adminPassword}"
const RESOLUTION = "${this.cameraResolution}"
const RECORDING_ENABLED = ${this.recordingEnabled}
...
<body>
    ...
    <div class="status-panel">
        ...
        <div id="statusInfo">
            ...
            <span>Resolution: ${this.cameraResolution}</span>
        </div>
    </div>
...
3. Go to: http://[PHONE_IP]:${this.serverPort}
4. Login with password: ${this.adminPassword}
...
- Port conflicts: Change port in server.js if ${this.serverPort} is in use
...
`
    ...
    // Write all files
    writeFileSync('/tmp/install.sh', installScript)
    writeFileSync('/tmp/server.js', serverCode)
    writeFileSync('/tmp/index.html', htmlInterface)
    writeFileSync('/tmp/setup_instructions.md', setupInstructions)
    writeFileSync('/tmp/package.json', packageJson)
    writeFileSync('/tmp/camera_utils.sh', cameraUtils)

    files.installScript = '/tmp/install.sh'
    files.serverCode = '/tmp/server.js'
    files.htmlInterface = '/tmp/index.html'
    files.setupInstructions = '/tmp/setup_instructions.md'
    files.packageJson = '/tmp/package.json'
    files.cameraUtils = '/tmp/camera_utils.sh'
...
// Generate access information
    const accessInfo = {
      ...
      networkUrl: `http://[PHONE_IP]:${this.serverPort}`,
      authRequired: true
    }

    $.export("$summary", `Successfully generated Termux CCTV setup with ${Object.keys(files).length} files. Server will run on port ${this.serverPort} with ${this.cameraResolution} resolution.`)

    return {
      success: true,
      message: "Termux CCTV setup package generated successfully",
      files: files,
      configuration: accessInfo,
      nextSteps: [
        "1. Download all generated files to your Android phone",
        "2. Install Termux from F-Droid or Play Store",
        "3. Grant camera and storage permissions",
        "4. Run install.sh in Termux",
        "5. Copy server files to ~/cctv-setup/",
        "6. Start server with ./start_cctv.sh",
        `7. Access camera at http://[PHONE_IP]:${this.serverPort}`
      ],
      security: {
        adminPassword: this.adminPassword,
        authenticationRequired: true,
        recommendation: "Change default password after setup"
      }
    }
  }
})
